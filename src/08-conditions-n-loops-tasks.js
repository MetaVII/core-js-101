...
function getSumBetweenNumbers(n1, n2) {
  let acc = n1;
  for (let index = n1 + 1; index <= n2; index += 1) {
    acc += index;
  }
  return acc;
}
...
function findFirstSingleChar(str) {
  for (let index = 0; index < str.length; index += 1) {
    ...
    if (str.indexOf(element) === index && str.indexOf(element, index + 1) === -1) {
      return element;
    }
  }
  return null;
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < ccn1.length; i += 1) {
    let cardNum = parseInt(ccn1[i], 10);

    if ((ccn1.length - i) % 2 === 0) {
      cardNum *= 2;
...
function getDigitalRoot(num) {
  ...
  let sum = 10;
  while (arr.length) {
    sum = arr.reduce((prev, cur) => prev + parseInt(cur, 10), 0);
    if (sum > 9) {
      arr = [...sum.toString()];
    } else {
      break;
...
function isBracketsBalanced(str) {
  ...
  for (let index = 0; index < str.length; index += 1) {
    ...
    if ('[{(<'.includes(element)) {
      stack.push(element);
    } else if (stack[stack.length - 1] === braces.get(element)) {
      stack.pop();
    } else {
      isError = true;
      break;
...
function getCommonDirectoryPath(pathes) {
  const arr = pathes.concat().sort();
  const shortest = arr[0];
  const longest = arr[arr.length - 1];
  const shortestArr = shortest.split('/');
  const longestArr = longest.split('/');
  let i = 0;

  while (shortestArr[i] === longestArr[i]) {
    i += 1;
  }

  if (i !== 0) {
    return `${shortestArr.slice(0, i).join('/')}/`;
  }
